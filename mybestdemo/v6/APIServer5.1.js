 ...
 * Base: APIServer5.0.js, one-line fix: get_irrigation_details now rounds
 * RecHrs (RecommendedIrrigationTime) to 1 decimal instead of passing the
 * raw CropManage float straight through.
        ...
        handler: async (args) => {
            ...
            for (const event of allEvents) {
                ...
                if (rec) {
                    ...
                    // FIX (APIServer5.1): round RecHrs to 1 decimal — was passing CropManage's
                    // raw float straight through (e.g. 0.6133333333333333).
                    event.RecommendedIrrigationTime        = parseFloat((parseFloat(rec.RecommendationSummary?.RecommendedIrrigationTime) || 0).toFixed(1));
