  ...
  // Version: 3.4.2 — Scenarios 1+2 in refreshSelectedIrrigations now use if(base) mcpCall
  // Pair with: 2. Src=/APIServer3.4.2.js (same CropManage paths for shared tools)
  ...
  async function refreshSelectedIrrigations() {
      ...
      try {
          ...
          if (selectedRanchId === 'ALL' && selectedPlantingId === 'ALL') {
              ...
              const base = getMcpBase();
              if (base) {
                  // MCP path — server handles all looping with stored token
                  const result = await mcpCall('get_irrigation_details', { ranchId: 'ALL' });
                  const events = Array.isArray(result.events) ? result.events : [];
                  loadEventsIntoDisplayRecords(events);
                  logToChat(`✅ ${events.length} irrigation events loaded via MCP`, 'M');
                  return;
              }

              // Offline fallback — direct fetch
              for (const ranch of ranchData) {
          ...
          // SCENARIO 2: Specific Ranch, ALL Plantings
              ...
              const base = getMcpBase();
              if (base) {
                  // MCP path — server gets all plantings for this ranch then loops irrigations
                  const result = await mcpCall('get_irrigation_details', { plantingId: 'ALL', ranchGuid: selectedRanchGuid });
                  const events = Array.isArray(result.events) ? result.events : [];
                  loadEventsIntoDisplayRecords(events);
                  logToChat(`✅ ${events.length} irrigation events loaded via MCP`, 'M');
                  return;
  ...
  async function postIrrigationToCropManage() {
      try {
          ...
          let eventId;
          if (getMcpBase()) {
              const resp = await mcpCall('post_new_irrigation', {
                  plantingId: String(plantingId),
                  eventdate: apiDate,
                  irrigationmethodid: payload.irrigationmethodid,
                  manageramountrecommendation: payload.manageramountrecommendation,
                  manageramountrecommendationhours: payload.manageramountrecommendationhours,
                  waterapplied: payload.waterapplied,
                  waterappliedhours: payload.waterappliedhours
              });
              result = resp;
              eventId = resp.id || resp.eventId || resp.Id || (resp.data && (resp.data.Id || resp.data.id));
          } else {
              const response = await fetch(url, {
                  method: 'POST',
  ...
  async function updateRecordToCropManage() {
      try {
          ...
          let result;
          if (getMcpBase()) {
              result = await mcpCall('update_irrigation', {
                  eventId: String(eventId),
                  eventDateMDY: apiDate,
                  mgrHours: parseFloat(selectedRecord.mgrHours) || 0,
                  appliedHours: parseFloat(selectedRecord.appliedHours) || 0,
                  irrigationMethodId: payload.IrrigationMethodId
              });
          } else {
              const response = await fetch(url, {
