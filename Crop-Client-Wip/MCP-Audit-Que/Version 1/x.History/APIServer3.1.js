/**
 * API MCP Server - CropManage Integration v3.1
 * HARNESS COPY - lives in 2. Src= folder for local testing
 * Run: node "2. Src=/APIServer3.1.js" from Version 1 folder
 * Port: 3101 (standalone)
 * Source: bumped from APIServer2.5.2.js
 * Tools: 17 (10 API + 7 JSON file services)
 *
 * Changes v2.5.2 → v3.0:
 *   - get_event_recommendation registered as named MCP tool (tool #9)
 *   - fetchEventRecommendation now calls through tool registry — no bare makeRequest
 *   - Matches api-component-3.0.js client — both announce v3.0 at runtime
 *   - get_event_recommendation: GET /v2/irrigation-events/{eventId}.json
 *   - Returns { success, event } — same shape client expects
 */
...
class MCPServer {
    ...
    start() {
        ...
        process.stderr.write('API MCP Server v3.1 (HARNESS) started - stdio mode\n');
    }
}
...
async function loadIntoDisplayRecords(args) {
    ...
    for (const event of result.events) {
        ...
        if (fullEvent) {
            ...
            // No recommendation found — use base event as-is
...
const apiServiceTools = [
    {
        ...
        inputSchema: {
            type: "object",
            properties: {
                username: { type: "string", description: "CropManage username" },
                password: { type: "string", description: "CropManage password" }
            },
    ...
    // set_token — client sends token to server after browser login
    // Required for local/online mode: server apiToken must be set before any API tool call
    {
        name: "set_token",
        description: "Store the CropManage API token server-side. Called by client immediately after get_token succeeds. Required before any CropManage API tool call in local/online mode.",
        inputSchema: {
            type: "object",
            properties: {
        ...
        handler: async (args) => {
            ...
            apiToken = args.token;
            console.log('✅ set_token: server apiToken stored (' + args.token.substring(0, 20) + '...)');
    ...
    },
    {
        ...
        description: "Fetch irrigation events, enrich with recommendations, return displayRecords. Accepts single plantingId OR plantings array (Irrigation-Selected-Array).",
        inputSchema: {
            type: "object",
            properties: {
                plantingId: { type: "string", description: "Single Planting ID (use this OR plantings array)" },
                plantings: { type: "array", description: "Irrigation-Selected-Array: [{selectedRanch, selectedRanchID, selectedPlanting, selectedPlantingID}]", items: { type: "object" } }
            },
            required: []
        },
        handler: async (args) => {
            // Build the planting list — either from array or single ID
            let plantingList = [];
            ...
            if (args.plantings && Array.isArray(args.plantings) && args.plantings.length > 0) {
                ...
                // Single plantingId — backwards compatible
                let pName = '';
                if (plantingsData) {
            ...
            // Work variable — accumulate displayRecords across all plantings
            let workRecords = [];

            for (const entry of plantingList) {
                // Set context for this planting
                ...
                const result = await loadIntoDisplayRecords({ plantingId: entry.plantingId });
                if (result.success && Array.isArray(result.displayRecords)) {
                    workRecords = workRecords.concat(result.displayRecords);
                }
            }

            // Final render — renumber sequential IDs, sort by ranch → planting → date
            workRecords.sort((a, b) => {
                if (a.ranch < b.ranch) return -1;
                if (a.ranch > b.ranch) return 1;
                if (a.planting < b.planting) return -1;
                if (a.planting > b.planting) return 1;
                // Parse date MM/DD/YY for sort
                const dA = a.scheduledDate || '';
                const dB = b.scheduledDate || '';
                return dA.localeCompare(dB);
            });
            workRecords.forEach((r, i) => r.id = i + 1);

            return {
                success: true,
                displayRecords: workRecords,
                count: workRecords.length,
                message: `Loaded ${workRecords.length} irrigation records from ${plantingList.length} planting(s)`
            };
        }
    },
    {
        ...
        inputSchema: {
            type: "object",
            properties: {
                plantingId: { type: "string", description: "Planting ID" }
            },
            required: []
        },
        handler: async (args) => {
    ...
    },
    {
        ...
        inputSchema: {
            type: "object",
            properties: {
                records: { type: "array", description: "Array of irrigation records" }
            },
            required: ["records"]
        },
        handler: async (args) => {
    ...
    // *** v3.0 GOLD TOKEN ***
    {
        ...
        description: "Fetch full recommendation for a single irrigation event by event ID. Returns enriched event data with RecommendedIrrigationAmount, ManagerAmountRecommendationHours, RecommendedInterval, and RecommendationSummary. Called after every POST to stamp real UC science values into the grid instantly.",
        inputSchema: {
            type: "object",
            properties: {
...
app.get('/ping', (req, res) => {
    res.json({
        status: 'alive',
        server: 'CropClient API MCP v3.0 (HARNESS)',
        mode: 'standalone',
...
// Start HTTP server
app.listen(PORT, () => {
    console.log(`API MCP Server v3.0 (HARNESS) running on port ${PORT}`);
