// ================================================================
// IRRIGATION_COMPONENT v2.1
// ================================================================
// NEW in 2.1: Click-to-filter on Ranch and Planting columns
// - Click Ranch or Planting cell → filters grid to matching records
...
//
// FILTER Tools (2) - NEW in v2.1:
//   1. setFilter - Set active filter (ranch or planting)
//   2. clearFilter - Remove all active filters
// ================================================================

// ========================================
// FILTER STATE (NEW in v2.1)
// ========================================
let activeFilter = null; // { type: 'ranch'|'planting', value: 'Ranch 1' }
...
// ========================================
// FILTER TOOLS (NEW in v2.1)
// ========================================

// ----------------------------------------
// FILTER TOOL #1: setFilter
// Sets active filter by type and value
// ----------------------------------------
function setFilter(filterType, filterValue) {
    if (!filterType || !filterValue) {
        return { returnCode: -1, statusMessage: 'Filter type and value required' };
    }
    
    if (filterType !== 'ranch' && filterType !== 'planting') {
        return { returnCode: -2, statusMessage: 'Filter type must be "ranch" or "planting"' };
    }
    ...
    return { 
        returnCode: 0, 
        statusMessage: `Filtering by ${filterType}: ${filterValue}`,
        data: { filterActive: true, type: filterType, value: filterValue }
    };
}

// ----------------------------------------
// FILTER TOOL #2: clearFilter
...
// ----------------------------------------
// FILTER TOOL #3: applyFilter
// Returns filtered records based on active filter
// ----------------------------------------
function applyFilter(records) {
    if (!activeFilter) {
        return records; // No filter active, return all
    }
    
    return records.filter(r => {
        if (activeFilter.type === 'ranch') {
            return r.ranch === activeFilter.value;
        } else if (activeFilter.type === 'planting') {
            return r.planting === activeFilter.value;
        }
        return true;
    });
}
        ...
        const mcpTools = {
            ...
            reset_table: {
                ...
                handler: function() {
                    ...
                    // Clear any active click filters
                    clearFilter();
            ...
            filter_by_ranch: {
                name: "filter_by_ranch",
                description: "Filter irrigation grid to show only records matching the specified ranch. Click same ranch again to clear filter.",
                inputSchema: {
                    type: "object",
                    properties: {
                        ranch: { type: "string", description: "Ranch name to filter by" }
                    },
                    required: ["ranch"]
                },
                handler: function(params = {}) {
                    if (!params.ranch) {
                        showResult('ERROR: ranch parameter required');
                        return { success: false, error: 'ranch required' };
                    }
                    
                    const result = setFilter('ranch', params.ranch);
                    renderTable();
                    showResult(result.statusMessage);
            ...
            filter_by_planting: {
                name: "filter_by_planting",
                description: "Filter irrigation grid to show only records matching the specified planting. Click same planting again to clear filter.",
                inputSchema: {
                    type: "object",
                    properties: {
                        planting: { type: "string", description: "Planting name to filter by" }
                    },
                    required: ["planting"]
                },
                handler: function(params = {}) {
                    if (!params.planting) {
                        showResult('ERROR: planting parameter required');
                        return { success: false, error: 'planting required' };
                    }
                    
                    const result = setFilter('planting', params.planting);
                    renderTable();
                    showResult(result.statusMessage);
            ...
            clear_filter: {
                name: "clear_filter",
                description: "Clear all active filters and show all irrigation records.",
                inputSchema: {
                    type: "object",
                    properties: {},
                    required: []
