  ...
  // Version: 3.0.2 — getToken() fixed: IF local/online → MCP get_token, ELSE → direct API

  console.log('=======================================================');
  console.log('  api-component-3.0.2 — LOADED');
  console.log('  getToken: Local/Online → MCP | Offline → direct API');
  console.log('=======================================================');
  ...
  async function getToken() {
      try {
          ...
          // Check mode: Local/Online use MCP, Offline use direct API
          const base = getMcpBase();
          let tokenData;

          if (base) {
              // --- Local/Online: Call get_token via MCP ---
              const mcpResult = await mcpCall('get_token', { username, password });
              if (!mcpResult || !mcpResult.success) {
                  throw new Error(`MCP get_token failed: ${mcpResult?.message || 'unknown error'}`);
              }
              tokenData = { access_token: mcpResult.token };
              logToChat(`✅ Token via MCP (${CC_STATE.mode} @ ${base})`, 'M');
          } else {
              // --- Offline: Call direct CropManage API ---
              ...
              logToChat(`✅ Token via direct API (offline)`, 'M');
          }
  ...
  async function getEventRecommendation(eventId) {
      ...
      if (base) {
          // --- Online/Local: MCP route ---
          const resp = await mcpCall('get_event_recommendation', { eventId: String(eventId) });
          // Server returns { success, event }
          if (resp && resp.success && resp.event) return { success: true, event: resp.event };
          // fallthrough to direct if MCP failed
          console.warn(`⚠️ MCP get_event_recommendation failed for ${eventId} — falling back to direct`);
      }
