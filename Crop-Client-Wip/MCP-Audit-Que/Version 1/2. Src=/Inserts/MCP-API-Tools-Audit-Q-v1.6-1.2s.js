...
<head>
    ...
    <title>CropClient MCP API Tools Audit v1.6</title>
    <style>
        :root {
            --left-width: 60%;
            --right-width: 40%;
        ...
        * {
        ...
        /* === S1 FIX: buttons pinned at bottom, content scrolls === */
        #Section-S1 {
            overflow: hidden !important;
            display: flex;
            flex-direction: column;
        }
        #Section-S1 .s1-scroll {
            flex: 1;
            overflow-y: auto;
            min-height: 0;
        }
        #Section-S1 .s1-footer {
            flex-shrink: 0;
        }
...
<body>
    <div class="container">
        <header>
            <h1>🌾 CropClient MCP Water Tools</h1>
        ...
        <div class="main-content">
            <div class="grid-container">
                <div id="Section-S1" class="grid-section">
                    <div class="s1-scroll">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 8px; flex-wrap: nowrap;">
                        <h2 style="margin: 0; white-space: nowrap;">🔐 Crop Manage API Audit</h2>
                        <div style="display: flex; align-items: center; gap: 6px; margin-left: auto; flex-wrap: nowrap;">
                            <label style="font-weight: 600; color: #333; font-size: var(--small-font); white-space: nowrap;">Login</label>
                            <input 
                                type="text" 
                                id="credentialsInput" 
                                style="width: 150px; min-width: 150px; max-width: 150px; padding: 5px; font-size: var(--small-font); border: 2px solid #ddd; border-radius: 6px;"
                            ...
                            <input
                                type="text"
                                id="urlDisplay"
                                style="width: 145px; min-width: 145px; max-width: 145px; padding: 5px; font-size: var(--small-font); border: 2px solid #ddd; border-radius: 6px; background: #f0f7f0; color: #2e7d32; font-weight: 600;"
                                readonly
                                value="http://api.cropclient.com"
                                title="Active MCP Server URL"
                            >
                            <select id="envSelect" title="Connection Mode" style="width: 78px; min-width: 78px; padding: 5px; font-size: var(--small-font); border: 2px solid #ddd; border-radius: 6px; background: #fff;">
                    ...
                    <!-- Hidden token kept for app logic -->
                    <div style="display:none;">
                        <input 
                            type="text" 
                            id="tokenDisplay" 
                            style="padding: 5px; font-size: var(--small-font); border: 2px solid #ddd; border-radius: 6px; background: #f5f5f5;"
                            readonly
                            placeholder="Token will appear here..."
                        >
                    </div>
                    <hr style="border: none; border-top: 2px solid #e0e0e0; margin: 4px 0;">
                    ...
                    <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 3px;">
                        ...
                        >
                            ...
                            <optgroup label="CropManage API">
                                <option value="token">🔐 Get Token</option>
                                <option value="ranches">🏠 Get Ranches</option>
                                <option value="plantings">🌱 Get Plantings</option>
                                <option value="irrigation_details">💧 Get Irrigation Details</option>
                                <option value="post_irrigation">📤 Post Irrigation</option>
                    ...
                    <div style="border: 1px solid #ddd; border-radius: 6px; overflow: hidden; display: flex; flex-direction: column; height: 140px; min-height: 140px; max-height: 140px; flex-shrink: 0; margin-bottom: 8px;">
                    ...
                    <div class="s1-footer" style="padding: 8px; margin-top: 8px; border-top: 2px solid #e0e0e0; display: flex; gap: 6px;">
                        <button onclick="saveDirect()" style="flex: 1; padding: 6px 8px; font-size: var(--small-font); background: #673AB7; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600;">💾 Save</button>
                        ...
                        <button onclick="restoreDirect()" style="flex: 1; padding: 6px 8px; font-size: var(--small-font); background: #009688; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600;">📂 Restore</button>
                    </div>
                </div>
                ...
                <div class="details-section">
                    ...
                    <div style="display: flex; gap: 6px; align-items: center;">
                        ...
                        >
                        <button onclick="sendChatPrompt()" class="send-button">🚀 Send</button>
                    </div>
                </div>
            </div>
            ...
            <div class="grid-container">
                <div class="grid-section">
                    <div class="record-count" style="display: flex; justify-content: space-between; align-items: center;">
                        <h3 style="margin: 0;">📊 <strong>Irrigation Records</strong> (<span id="recordCount">0</span>)</h3>
...
<script>
// direct MCP save/restore only
function getDirectMcpBase() {
    const mode = (document.getElementById('envSelect')?.value || '').toLowerCase();
    return mode === 'local' ? 'http://localhost:3101' : 'https://api.cropclient.com';
}
...
async function saveDirect() {
    ...
    try {
        await fetch(base + '/tools/data_operation', {
            ...
            body: JSON.stringify({ action: 'write', table: 'steve', data: displayRecords })
        });
        showResult('✅ Saved to steve');
    } catch (e) {
...
async function restoreDirect() {
    ...
    try {
        const res = await fetch(base + '/tools/data_operation', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'read', table: 'steve' })
        });
        const json = await res.json();
        const rows = Array.isArray(json.data) ? json.data : [];

        // front door: restore source JSON, then run normal app load
        irrigationData = JSON.parse(JSON.stringify(rows));
        selectedRecord = null;
        read();

        showResult('✅ Restored from steve');
    } catch (e) {
        showResult('❌ Restore failed: ' + e.message);
    }
}
</script>
...
<!-- @@@@ IRRIGATION_COMPONENT v2.0 id:irrigation-mcp @@@@ -->
<!-- ================================================================
...
// ========================================
        ...
        /* BEGIN MCP MINI MODULE: host + token handshake (v1.2)
           ...
           How to use (type into Prompt box, then click Send):
             ...
             - "mcp host http://api.cropclient.com"               (set hosted MCP base)
        ...
        */
        // --- MCP Connection Mode (Harness Runtime State) ---
// Single source of truth while the harness is running.
// Online is the default for server installs.
const CC_STATE = {
            mode: 'online',                // 'local' | 'online' | 'offline'
            localBase: 'http://localhost:3101',
            onlineBase: 'http://api.cropclient.com',
            overrideBase: ''               // optional: set by "mcp host <url>"
        };

        function syncUrlDisplay() {
            const el = document.getElementById('urlDisplay');
            if (!el) return;
            const base = getMcpBase();
            el.value = base || '(offline)';
            el.style.color = base ? '#2e7d32' : '#999';
        }

        function setEnvMode(mode) {
            const m = (mode || '').toLowerCase();
            if (m === 'local' || m === 'online' || m === 'offline') {
                CC_STATE.mode = m;
            }
            // Keep UI synced (no alerts, just facts)
            const sel = document.getElementById('envSelect');
            if (sel && sel.value !== CC_STATE.mode) sel.value = CC_STATE.mode;
            syncUrlDisplay();
}

        function getMcpBase() {
            if (CC_STATE.mode === 'offline') return null;
            const base = (CC_STATE.overrideBase && CC_STATE.overrideBase.trim())
                ? CC_STATE.overrideBase.trim()
                : (CC_STATE.mode === 'online' ? CC_STATE.onlineBase : CC_STATE.localBase);
            return base.replace(/\/+$/, '');
        }

        // Kept for backward compatibility with the existing "mcp host <url>" command.
        // This does not block or alert; it just updates state.
        function setMcpBase(value) {
            const v = (value || '').trim().replace(/\/+$/, '');
            if (!v) return null;
            CC_STATE.overrideBase = v;

            // If override looks like localhost, align mode to local; otherwise online.
            if (v.includes('localhost') || v.includes('127.0.0.1')) setEnvMode('local');
            else setEnvMode('online');

            syncUrlDisplay();
            return v;
        }

        // Bind the dropdown — HTML 'selected' attribute sets the default.
        document.addEventListener('DOMContentLoaded', () => {
            const sel = document.getElementById('envSelect');
            if (sel) {
                setEnvMode(sel.value);  // respect whatever HTML says
                sel.addEventListener('change', (e) => {
                    CC_STATE.overrideBase = ''; // dropdown selection wins
        ...
        async function mcpSendTokenAndValidate(mode = 'ranches') {
            ...
            if (!apiToken || !String(apiToken).trim()) {
                showResult('❌ No apiToken in the app yet. Run Get Token first.');
                return;
            }
            ...
            try {
                ...
                logToChat('✅ MCP stored token (set_token)', 'M');
            } catch (e) {
                logToChat(`❌ MCP set_token failed: ${e.message}`, 'M');
                showResult('❌ MCP set_token missing or failed. Add set_token tool to APIServer1.2.js.');
                return;
            }
            ...
            try {
                if (mode === 'refresh') {
                    ...
                    if (selectedPlantingId && selectedPlantingId !== 'ALL') {
                        ...
                        logToChat(`✅ MCP irrigation details ok (plantingId=${selectedPlantingId})`, 'M');
                        showResult(`✅ MCP validated token. get_irrigation_details returned ${(resp.data && resp.data.length) || 0} records`);
                    } else {
                        const ranches = await mcpCall('get_ranches', {});
                        logToChat('✅ MCP get_ranches ok', 'M');
                        showResult(`✅ MCP validated token. get_ranches returned ${(ranches.data && ranches.data.length) || 0} ranches`);
                    }
                } else {
                    const ranches = await mcpCall('get_ranches', {});
                    logToChat('✅ MCP get_ranches ok', 'M');
                ...
                logToChat(`❌ MCP validation call failed: ${e.message}`, 'M');
                showResult(`❌ MCP validation failed: ${e.message}`);
            }
        }
        /* END MCP MINI MODULE */
        ...
        function sendPrompt() {
            ...
            if (prompt.startsWith('mcp host ')) {
                ...
                if (saved) {
                    logToChat(`ℹ️ MCP host override: ${saved} (mode: ${CC_STATE.mode})`, 'M');
                    showResult(`ℹ️ MCP host override set: ${saved} (mode: ${CC_STATE.mode})`);
                } else {
                    logToChat(`ℹ️ MCP host override ignored (blank). Mode remains: ${CC_STATE.mode}`, 'M');
                    showResult(`ℹ️ MCP host override ignored (blank). Mode: ${CC_STATE.mode}`);
                }
                return;
            }
