/**
 * APIServer3.4.2.js — CropManage MCP Server (same folder as HTML src= scripts)
 * Source: 2. Src=/APIServer3.4.2.js | Deploy: crop-client-services/api-server/
 * Run: from Version 1 folder: node "2. Src=/APIServer3.4.2.js" | Port: 3101
 *
...
const apiServiceTools = [
    ...
    },
    {
        ...
        description: "Fetch irrigation events, enrich with recommendations, return events. Accepts ranchId:'ALL' (Scenario 1), plantingId:'ALL'+ranchGuid (Scenario 2), plantings array, or single plantingId.",
        inputSchema: {
            type: "object",
            properties: {
                ranchId:    { type: "string", description: "'ALL' — Scenario 1: all ranches, all plantings" },
                ranchGuid:  { type: "string", description: "Ranch GUID — required for Scenario 2 (plantingId:'ALL')" },
                plantingId: { type: "string", description: "Single planting ID or 'ALL' (Scenario 2 requires ranchGuid)" },
        ...
        handler: async (args) => {
            ...
            // ranchId:'ALL' → loop all ranchData, get plantings for each via mcpCall
            if (args.ranchId === 'ALL') {
                if (!ranchData || ranchData.length === 0) {
                    return { success: false, message: 'No ranch data on server. Call get_ranches first.' };
                }
                for (const ranch of ranchData) {
                    try {
                        let plantings = [];
                        if (base) {
                            const resp = await mcpCall('get_plantings', { ranchGuid: ranch.Ranch_External_GUID });
                            plantings = Array.isArray(resp.plantings) ? resp.plantings : [];
                        } else {
                            const resp = await getPlantingsByRanch(ranch.Ranch_External_GUID);
                            plantings = Array.isArray(resp.plantings) ? resp.plantings : [];
                        }
                        plantings.forEach(p => {
                            plantingList.push({
                                plantingId:   String(p.Id),
                                plantingName: p.Name || '',
                        ...
                        console.warn(`⚠️ Scenario 1: failed plantings for ${ranch.Name}: ${e.message}`);
                    }
                }

            // ---- SCENARIO 2: Specific Ranch, ALL Plantings -------------------------
            // plantingId:'ALL' + ranchGuid → get all plantings for that ranch via mcpCall
            } else if (args.plantingId === 'ALL' && args.ranchGuid) {
                let plantings = [];
                if (base) {
                    const resp = await mcpCall('get_plantings', { ranchGuid: args.ranchGuid });
                    plantings = Array.isArray(resp.plantings) ? resp.plantings : [];
                } else {
                    const resp = await getPlantingsByRanch(args.ranchGuid);
                    plantings = Array.isArray(resp.plantings) ? resp.plantings : [];
                }
                plantings.forEach(p => {
                    plantingList.push({
                        plantingId:   String(p.Id),
                        plantingName: p.Name || '',
                        ranchName:    selectedRanchName,
                        ranchId:      selectedRanchId
                    });
                });

            // ---- existing: explicit plantings array --------------------------------
            } else if (args.plantings && Array.isArray(args.plantings) && args.plantings.length > 0) {
            ...
            // ---- existing: single plantingId ---------------------------------------
                ...
                return { success: false, message: 'ranchId:ALL, plantingId:ALL+ranchGuid, plantings[], or plantingId required' };
            }

            if (plantingList.length === 0) {
                return { success: true, events: [], count: 0, message: 'No plantings found for the selection' };
            }
            ...
            // Step 2 — enrichment: get_event_recommendation per event
            ...
            // Return enriched raw events — client handles render
            return {
                success: true,
                events: allEvents,
