/**
 * APIServer3.2.js — CropManage MCP Server
 * Source: 2. Src= (truth copy) | Deploy: crop-client-services/api-server/
 * Run: node APIServer3.2.js | Port: 3101
 *
 * Changes v3.1 → v3.2:
 *   - Token display removed from set_token (no token substring logged to console)
 *   - Fresh injection of api-component-3.0.js (getMcpBase clean, getEventRecommendation fixed)
 *   - CC_STATE removed from mcp-engine — api-component owns it
 *   - getMcpBase() now reads mcpServerUrl directly from mcp-engine
 *   - getEventRecommendation: MCP failure returns error, no silent fallthrough to direct fetch
 */
...
class MCPServer {
    ...
    start() {
        ...
        process.stderr.write('APIServer3.2 started - stdio mode\n');
    }
}
...
// Injected from: 2. Src=/api-component-3.0.js
...
async function postNewIrrigation(args) {
    ...
    try {
        ...
        if (response.statusCode === 200 || response.statusCode === 201) {
            return { success: true, eventId: response.data.Id, message: 'Irrigation event posted successfully' };
        }
        return { success: false, message: `POST failed: ${response.statusCode}` };
    } catch (error) {
        return { success: false, message: error.message };
    }
}
...
async function batchPostQueue(records) {
    ...
    for (const record of records) {
        if (record.eventId) {
            results.push({ record, result: await updateIrrigationEvent(record.eventId, record), action: 'UPDATE' });
        } else {
            results.push({ record, result: await postNewIrrigation(record), action: 'POST' });
        }
    }
...
const apiServiceTools = [
    ...
    },
    {
        ...
        handler: async (args) => await updateIrrigationEvent(args.eventId, args)
    },
    {
...
app.get('/ping', (req, res) => {
    res.json({
        status: 'alive',
        server: 'CropClient APIServer3.2',
        mode: 'standalone',
...
// Start HTTP server
app.listen(PORT, () => {
    console.log(`APIServer3.2 running on port ${PORT}`);
