...
---George please finish this spec as v3 with the staging included 
   all tokens staged in irrigationss its the UI wiz... 
Coach ====================================================== read this first. 

C:\AICode\CropClient-Claude-Desktop\Crop-Client-Wip\MCP-Audit-Que\Version 1\1. Task Lists
...
NO tool logic in HTML. NO callTool scattered. ONE engine.
 ...
 scratch all the CallTool references.  this is how we replace it with mcpEngine-Staging

 Irrigation-component will do the staging that calltool used to do. 

the 
irrigation-component owns the whole chain:

get_token
get_ranches
get_plantings
get_irrigation_details
All irrigation tokens. One component. Click handlers inside it call mcpEngineSend for each step in the chain.

so it has the the clicks are like button pushes in the crud. right ?

YES. Exactly.
this need to talk about irrigation doing the staging and called by send(staging, ,,)

Ranch click = button push → calls mcpEngineSend('get_plantings')  i.e .. (staging(get_plantings, plantingid: 2233)) or whatever. 
Planting click = button push → calls mcpEngineSend('get_irrigation_details')
Create button = button push → calls mcpEngineSend('create_next_irrigation')
Update button = button push → calls mcpEngineSend('update_record')

All in irrigation-component. Same pattern as CRUD buttons today.
